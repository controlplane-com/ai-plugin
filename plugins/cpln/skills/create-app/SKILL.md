...
- Limits follow GitHub's: 100 MB per file, 1 GB per app, 20,000 files per app; 200 files and 100 MB per call (far more than one reply can hold). A file over 1 MB is not returned by `get_app_files` (the download archive has it). Generated and vendored files stay out; dependencies are installed by the build.
- Paths are relative with forward slashes (`src/index.js`); `..`, absolute paths, `node_modules/`, and `.git/` are refused.
- Binary files go as `encoding: "base64"` when you have their bytes, or through `uploadPaths` when the user has them. Edits apply to text files only.
...
| `mcp__cpln__get_app_files` | List the files, read one, or get a download link (`download: true`). |
