...
- **Capacity AI** runs on every workload type, but it is off with CPU-utilization or multi-metric autoscaling. Check the stored `spec.defaultOptions.capacityAI` before blaming the tuner. On cron the new reservation only lands at the next execution.
...
- `capacityAI` with `metric: cpu` is rejected, as is `capacityAI: true` with a GPU container; every workload type otherwise accepts it.
