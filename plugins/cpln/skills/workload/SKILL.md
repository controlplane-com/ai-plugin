...
|---|---|---|
| `containers[]` — `image`, `ports`, `cpu`/`memory`, `env`, `command`/`args`, probes, `metrics`, `volumes` | the containers | `create_workload` / `update_workload` |
...
Always call `get_resource_schema` for the workload kind before authoring a spec — never hand-write fields from memory.
...
6. `mcp__cpln__create_workload` / `mcp__cpln__update_workload` (PATCH — only sent fields change), plus `configure_workload_*` for load balancer / sidecar / extras / local options.
...
| `mcp__cpln__update_workload` | Update a workload (PATCH — only sent fields change). |
