...
3. Provision in dependency order, MCP-first via the typed `create_*` tools (GVC, then identities, volumesets, workloads, domains; secrets first with `create_secret`: generated values, or a Console link for the user's own); fall back to `cpln apply -f` when MCP is unavailable or in CI/CD.
