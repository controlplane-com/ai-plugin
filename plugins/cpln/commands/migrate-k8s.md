...
3. Provision in dependency order, MCP-first via the typed `create_*` tools (GVC, then identities, volumesets, workloads, domains; secrets are created by the user first); fall back to `cpln apply -f` when MCP is unavailable or in CI/CD.
