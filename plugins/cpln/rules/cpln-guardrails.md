...
- Pull secrets are configured at the **GVC level**, not per workload. Only `dockerhub`, `ecr`, and `gcp` secret types qualify.
...
On an org-not-found error from any tool, **stop and ask the user**. Do not retry with capitalization, hyphen, or pluralization variants — those are hallucinations, not recovery.

For **read-only** commands (`get`, `query`, `audit`, `logs`, `permissions`, `access-report`, `eventlog`), defaulting to the active/recently-named profile is acceptable — but **announce the target before running**: _"Using profile `<name>` → org `<org>`, GVC `<gvc>`…"_ — so the user can correct course before output is produced.
...
- Your own org's registry: use `//image/NAME:TAG` (or the equivalent `/org/<org>/image/<name>:<tag>`) in workload specs. `<own-org>.registry.cpln.io/...` is only for `docker login` / `docker push`, never in specs.
...
**Organizations are immutable.** Once created, they cannot be deleted via the API, CLI, or any MCP tool — removal requires Control Plane support. There is no `cpln org delete` (and no `delete_org` MCP tool), and the org-scoped API at `https://api.cpln.io` cannot create or delete orgs (org creation lives on a different endpoint).
...
- Reaching for `force-redeployment` to "try again" after a failed deploy hides the real problem. Diagnose first (`cpln workload get-deployments` / `get_workload_deployments`, `cpln logs` / `get_workload_logs`), fix the manifest, then re-apply.
...
- For cron workloads, `suspend = true` is the documented pause path; pair with `cpln workload cron start` / `workload_start_cron` for manual on-demand runs.
...
Always **draft new specs from the OpenAPI definitions** (`cpln+virtual://openapi`, or `cpln <resource> manifest`), not from existing environment-specific resources that may have drifted.

### Destructive Operations — Always Confirm With Blast Radius

Some operations cannot be undone, or have effects that reach beyond the resource being changed. **Before any destructive operation listed below, the AI MUST present a structured summary AND wait for explicit user confirmation — even when permissions are set to bypass / auto-approve.** Permission mode is about the host UX; this rule is conversation-level safety and is independent. Bypass permissions does NOT authorize destructive product operations.

**Always destructive (data or resource loss):**

- Any `cpln <resource> delete` / `delete_*` tool — workloads, GVCs, secrets, policies, identities, volumesets, domains, images, etc.
- `cpln gvc delete-all-workloads`
- `cpln volumeset shrink` / `shrink_volumeset` — provisions a NEW volume at the smaller size; existing data is **NOT** migrated; the old volume is permanently deleted. Only available on `ext4` and `xfs` filesystems (not `shared`). **Only safe** for workloads with built-in data redundancy: Apache Kafka with proper replication factor, distributed databases with replication (Cassandra, CockroachDB, etc.), or anything where data can be rebuilt from other replicas. **Never** for single-replica stateful workloads, databases without replication, or any application where data loss is unacceptable.
- `cpln volumeset snapshot delete` / `delete_volumeset_snapshot`, `cpln volumeset volume delete` / `delete_volumeset_volume`
- `restore_volumeset_snapshot` — overwrites the current volume state (any data written since the snapshot is lost)
...
- `cpln gvc remove-location` / `remove_gvc_locations` — forces redeployment of any workloads in that location
...
>
> - **Action**: `<exact command(s) or tool call(s)>`
...
> - **Constraint**: `<exact technical limitation, citing the rule from "Workload Type Constraints" above or the relevant skill>`
...
- The component genuinely isn't in the catalog — then build a workload, but do it production-grade per the **"Production-Grade Workload Defaults"** rule above.
...
**Never write a cpln command from memory.** See `rules/cli-conventions.md` for CLI structure, resource command map, and hallucination traps. If you cannot cite a verified flag, state that you cannot verify the command instead of guessing.

### Best Practices

- For waits: see the **"Long-Running Operations"** rule above. First-deploys → apply with the patience-windowed safety net. Repeat deploys of known-good workloads → `cpln apply --file <m>.yaml --ready` is fine. Never plain polling from the AI layer.
...
- **Discover before mutating.** For GVC and location operations, include a discovery step in the plan: list GVCs or locations first, verify the target org/GVC/location exists, and only then perform the mutation based on that live context.
- **Compact overviews, not full JSON.** When summarising volumesets, GVCs, locations, or workloads to the user, surface meaningful metadata (name, alias, status, geo, locations, created time) instead of dumping entire JSON payloads.
- **Concise, structured answers** (plan → steps → verify) over long narratives — downstream AI agents can turn structured guidance into actions; they cannot easily parse prose.
...
- API: https://api.cpln.io (treat https://api.cpln.io/openapi.json as the definitive schema; base all paths on `/org/{org}/...` and add `/gvc/{gvc}/` when resources are GVC-scoped)
