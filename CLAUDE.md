...
- `.claude-plugin/marketplace.json` — Claude marketplace entry. `source` is the relative-path string `"./plugins/cpln"` (resolved against the marketplace root, which is the repo root). Don't use a `github`-typed source object with a `path` field — Claude's `github` source schema only accepts `repo`, `ref`, and `sha`; an extra `path` is silently ignored and the whole repo gets cached with no `plugin.json` at the cache root, so skills/agents/commands disappear.
...
- `hooks/hooks.json` — **Gemini-only** hook config. Registers a `SessionStart` rule injector (Gemini accepts the Claude-style `matcher` + `hookSpecificOutput.additionalContext` shape; events live on `HookEventName` in `gemini-cli/packages/core/src/hooks/types.ts`). The command reuses `plugins/cpln/hooks/inject-rules.sh` by setting `CLAUDE_PLUGIN_ROOT="${extensionPath}/plugins/cpln"` so the script's rule walk works the same way as on Claude/Codex. Gemini auto-discovers this file from the extension dir; the path is hardcoded in `packages/cli/src/config/extension-manager.ts` and cannot be redirected via the manifest. Redundant with `GEMINI.md` (which `contextFileName` loads automatically) — kept as a safety net so a `GEMINI.md` drift doesn't silently lose the guardrails.
...
- `rules/` — Validation guardrails and manifest references. Files with `alwaysApply: true` in frontmatter (`cli-conventions.md`, `cpln-guardrails.md`) are concatenated and injected into every Claude **and Codex** session by the `SessionStart` entry in `hooks/cpln-hooks.json`. `alwaysApply` is not a native field — the hook is what gives it meaning. Codex works out of the box because it sets `CLAUDE_PLUGIN_ROOT` for OOTB compatibility (verified in `codex-rs/hooks/src/engine/discovery.rs`) and accepts the same `hookSpecificOutput.additionalContext` schema. Gemini gets the same rules content from `GEMINI.md` via `contextFileName`. **Codex caveat:** plugin-bundled hooks are gated by the `plugin_hooks` feature, which is `stage: UnderDevelopment, default_enabled: false` in `codex-rs/features/src/lib.rs`. When the flag is off, the `/plugins` → Control Plane → Hooks row reads "No plugin hooks." and the `SessionStart` injector never runs — even though the file is loaded without errors. Users have to add `[features]\nplugins = true\nplugin_hooks = true` to `~/.codex/config.toml`; the README's Codex install section spells this out.
...
- `hooks/inject-rules.sh` — Bash script the `SessionStart` hook invokes (`bash "$CLAUDE_PLUGIN_ROOT/hooks/inject-rules.sh"`). Walks `rules/*.md`, picks the ones whose frontmatter has `alwaysApply: true`, concatenates them, and emits the `{hookSpecificOutput:{hookEventName,additionalContext}}` envelope on stdout. Uses `awk` for JSON escaping (POSIX-mandated, on every Unix) instead of `jq`, which isn't installed by default on macOS or minimal containers. Must stay executable (`chmod +x`); never exit non-zero so a missing dependency can't block session startup.
